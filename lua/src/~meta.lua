...
function meta.is_type(x)
    ...
    return x.__meta ~= nil and x.__meta.typename == "Type"
end
...
--- @brief Set initial value of property of meta.Type
--- @param type meta.Type
--- @param property_name string
--- @param value any
--- @returns void
function meta.set_property_value(type, property_name, value)

    if not meta.is_type(type) then
        error("[ERROR] In meta.set_property_value: Object is not a type")
    end

    if not meta.has_property(type, property_name) then
        error("[ERROR] In meta.set_property_value: Type `" .. type.name .. "` does not have a property `" .. property_name .. "`")
    end

    type.properties[property_name] = value
end
...
function meta._test()
    ...
    test.assert_that("is_enum_value", not meta.is_enum_value(enum, -1))
    test.end_test()
end